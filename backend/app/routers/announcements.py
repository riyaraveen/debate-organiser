...
def create_announcement(body: AnnouncementIn, db: Session = Depends(get_db), current_user: User = Depends(require_admin)):
    a = Announcement(title=body.title, content=body.content, created_by=current_user.id)
    db.add(a)
    db.commit()
    db.refresh(a)
    return a
